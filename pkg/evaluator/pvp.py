...
class PVPEvaluator(CompetitionEvaluator):
    ...
    def get_final_metrics(self, history, mode):
        # policy_ids = set()
    ...
    def evaluate(shared_dir: str,
        ...
        contenders = PVPEvaluator.load_contenders(shared_dir, grader_ids,
                                                    api_token, min_score,
                                                    deadline,
                                                    specific_submissions)
        contenders = {c.id: c for c in contenders}
        
       
        # read the policy_store as contenders
        # contenders = {}
        # if not os.path.exists( policy_store_dir ):
        #     return []
        # sub_pts = os.listdir( policy_store_dir )
        # for k, sub_pt in sub_pts:
        #     participant_name, track = sub_pt.split("_")
        #     contenders[k] = participant_name
        
        mode = Mode.PVP
        # TODO:重写这个，合并的逻辑
        history = CompetitionEvaluator.load(shared_dir, rollout_name)
        # TODO:生成最终结果的逻辑需要合并
        final = PVPEvaluator.get_final_metrices(history, mode)
        ...
        upload_ret = {}
        if upload_replay:
            # rename submission_ids in replays
            replays = glob.glob(f"{shared_dir}/replays/*.lzma")
            rename_replay = bool(int(os.getenv("NMMO_RENAME_REPLAY", "1")))
            if rename_replay:
                for path in replays:
                    try:
                        from nmmo import Replay
                        replay = Replay.load(path)
                        replay = util.submission_id_to_name(
                            replay, {
                                k: v.participant_name
        ...
        if dry_run:
            ...
            ranked = sorted(ranked,
                            ...
                            reverse=True)
            print(",".join(ranked))
            print("submission_id\tparticipant\tsr\tTotalScore")
            for i, submission_id in enumerate(ranked):
                print(
                    f"{i+1}\t{submission_id}\t{contenders[submission_id].participant_name}\t{final[submission_id]['TrueSkillMu']:.2f}\t{final[submission_id]['TotalScore']:.2f}"
                )
            return final

        
        api = API(api_token)
        for submission_id, result in final.items():
            logger.info(f"update submission {submission_id}")
            submission: AIcrowdSubmission = util.guarantee(api.get_submission)(
                contenders[submission_id].grader_id, submission_id)
            ...
            submission.meta = {
                **meta,
                **{f"PVP-{tournament}-{k}": v
                   for k, v in result.items()}
            }
            submission.meta[f"Replay-PVP-{tournament}"] = upload_ret.get(
                "submissions", {}).get(submission_id, "none")
